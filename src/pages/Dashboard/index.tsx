...
const Dashboard: React.FC = () => {
  ...
  async function handleUpdateFood(
    ...
    try {
      await api.put(`/foods/${editingFood.id}`, food).then(() => {
        setFoods(
          foods.map(foundFood => {
            if (foundFood.id === editingFood.id) {
              return {
                ...food,
                id: editingFood.id,
  ...
  async function handleDeleteFood(id: number): Promise<void> {
    ...
    const foodToBeRemoved = foods.find(food => food.id === id);
    ...
    await api.delete(`/foods/${id}`).then(() => {
      const updatedFoodList: IFoodPlate[] = foods.filter(
        food => food.id !== id,
      );
